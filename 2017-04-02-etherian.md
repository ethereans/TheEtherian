...
- >[Brendan Eich]
 Today, we are announcing a new blockchain-based digital advertising platform using an innovative, Ethereum-based unit of exchange called the Basic Attention Token (BAT).
...
- >[Vitalik Buterin] so I realized that the viper contract may need a bit more work [Vitalik Buterin] basically, the challenge is this scenario [Vitalik Buterin] you see 2/3 prepares [Vitalik Buterin] you commit [Vitalik Buterin] then there is a short-range fork, and in the fork 2/3 prepares don't appear until the epoch ends [Vitalik Buterin] right now, those prepares cannot get included [Vitalik Buterin] so you need to be able to process prepares even if they come too late [Vitalik Buterin] at least for the purpose of evaluating slashing conditions on commits [Vitalik Buterin] what this also implies is that you need to be able to tell at any point in the future whether or not a given validator was in a given dynasty.
---
...
- **Update on the alpha coming on Thursday**. 
